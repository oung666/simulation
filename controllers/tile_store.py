...
class TileStore:
    ...
    def best_zoom_for(self, requested_zoom: float) -> int | None:
        """根据当前地图 zoom，选择不高于当前缩放的最佳本地瓦片 zoom。"""
        zooms = self.available_zooms()
        if not zooms:
            return None
        lower_or_equal = [zoom for zoom in zooms if zoom <= requested_zoom]
        if lower_or_equal:
            return lower_or_equal[-1]
        return zooms[0]

    def fallback_zooms_for(self, requested_zoom: float) -> list[int]:
        """Return candidate tile zooms from best match down to coarser fallbacks."""
        zooms = self.available_zooms()
        if not zooms:
            return []
        lower_or_equal = [zoom for zoom in zooms if zoom <= requested_zoom]
        if lower_or_equal:
            return list(reversed(lower_or_equal))
        return [zooms[0]]
