    ...
    for attempt in range(retries + 1):
        try:
            request = Request(
                url,
                headers={
                    "User-Agent": "simulation_offline_tile_prepare/0.1",
...
def parse_args() -> argparse.Namespace:
    ...
    return argparse.ArgumentParser(description="Download offline XYZ raster tiles for simulation.").parse_args()


def main() -> int:
    parser = argparse.ArgumentParser(description="Download offline XYZ raster tiles for simulation.")
    root = Path(__file__).resolve().parents[1]
